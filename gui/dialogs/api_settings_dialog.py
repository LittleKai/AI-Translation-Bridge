...
class APISettingsDialog:
    ...
    def setup_ui(self):
        ...
        for key in self.config.get('keys', []):
            masked_key = f"{key[:10]}..." if len(key) > 10 else key
    ...
    def add_key(self):
        ...
        key_entry.grid(row=1, column=0, pady=(5, 10))
    ...
    def remove_key(self):
        ...
        if selection:
            ...
            if 'keys' in self.config and index < len(self.config['keys']):
                del self.config['keys'][index]
