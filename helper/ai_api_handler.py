...
class AIAPIHandler:
    ...
    def call_perplexity_api(self, prompt, model_name, config, api_keys):
        """Call Perplexity API"""
        api_key = self.get_random_api_key(api_keys)
        if not api_key:
            error_msg = "No available API keys for Perplexity"
            self.main_window.log_message(f"Error: {error_msg}")
            return None, error_msg
        ...
        try:
            self.main_window.log_message(f"Calling Perplexity API with model: {model_name}")
            response = requests.post(url, headers=headers, json=payload, timeout=30)
            ...
            if response.status_code == 200:
                ...
                if 'choices' in result and result['choices']:
                    text = result['choices'][0]['message']['content']
                    self.main_window.log_message("Perplexity API call successful")
                    return text, None
                else:
                    error_msg = "No valid response from Perplexity API"
                    self.main_window.log_message(f"Error: {error_msg}")
                    return None, error_msg
            else:
                error_msg = f"Perplexity API error - Status: {response.status_code}, Response: {response.text}"
                self.main_window.log_message(f"Error: {error_msg}")
                if response.status_code in [401, 403, 429]:
                    self.failed_keys.add(api_key)
                    self.main_window.log_message(f"API key marked as failed: {api_key[:10]}...")
            ...
            error_msg = "Perplexity API timeout (30s exceeded)"
            self.main_window.log_message(f"Error: {error_msg}")
            return None, error_msg
        except Exception as e:
            error_msg = f"Perplexity API exception: {str(e)}"
            self.main_window.log_message(f"Error: {error_msg}")
            return None, error_msg
